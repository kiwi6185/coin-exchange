...
export default {
  ...
  methods: {
    async _getList() {
      ...
      const res = await this.listCallback()
    ...
    exportExcel(formName, url, fileName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const { ruleForm, current, size } = this.$refs[formName].$parent
          const params = {}
          ...
          commonApi.exportExcel(url, params).then((res) => {
            const filename = fileName || decodeURIComponent(res.headers['content-filename'])
    ...
    async beforeUpload() {
      const res = await uploadApi.getPreUpload()
      if (res) {
        const preUploadData = res
    ...
    handleUploadSuccess(response) {
      ...
      const aliyunFileUrl = 'https://caseythekiwi-coin-exchange-imgs.oss-cn-guangzhou.aliyuncs.com/'
      if (status === 'OK') {
